...
def _cognito_users(template) -> dict:
    return template.find_resources("AWS::Cognito::UserPoolUser")


def _group_attachments(template) -> dict:
    return template.find_resources("AWS::Cognito::UserPoolUserToGroupAttachment")


def test_staging_creates_the_accounts_the_e2e_suite_signs_in_as(data):
    users = _cognito_users(data[STAGING.name])
    assert len(users) == len(STAGING.test_users)

    created = {u["Properties"]["Username"] for u in users.values()}
    assert created == {u.email for u in STAGING.test_users}


def test_production_has_no_seeded_accounts(data):
    """The guard that matters.

    A seeded account with a machine-readable password sitting in Secrets Manager
    is a way in. Staging having one is a convenience; production having one is a
    finding. If this fails, the answer is not to change the assertion.
    """
    template = data[PROD.name]
    assert _cognito_users(template) == {}
    assert _group_attachments(template) == {}

    secrets = template.find_resources("AWS::SecretsManager::Secret")
    for secret in secrets.values():
        name = secret["Properties"].get("Name", "")
        assert "test-user" not in str(name)


def test_only_the_admin_account_joins_the_admin_group(data):
    attachments = _group_attachments(data[STAGING.name])
    expected = [u for u in STAGING.test_users if u.admin]

    assert len(attachments) == len(expected)
    for attachment in attachments.values():
        assert attachment["Properties"]["GroupName"] == "Admin"

    # A suite whose only account is an admin cannot notice admin-only UI leaking
    # into a customer's view, because that account is allowed to see all of it.
    assert any(not u.admin for u in STAGING.test_users)


def test_the_group_attachment_waits_for_the_user_and_the_group(data):
    """CloudFormation infers no ordering between these three, and attaching
    before either exists fails the deploy."""
    template = data[STAGING.name]
    attachments = _group_attachments(template)
    assert attachments

    for attachment in attachments.values():
        depends = attachment.get("DependsOn", [])
        depends = depends if isinstance(depends, list) else [depends]
        assert len(depends) == 2, depends


def test_no_test_user_password_is_written_into_the_template(data):
    """The password is generated by Secrets Manager, never authored here.

    If someone replaces the generator with a literal, this catches it: the value
    would then be readable in the template, in `cdk diff`, and in the
    ...
    seeded = {
        logical: secret
        for logical, secret in secrets.items()
        if "test-users" in str(secret["Properties"].get("Name", ""))
    }
    assert len(seeded) == len(STAGING.test_users)

    for secret in seeded.values():
        properties = secret["Properties"]
        assert "SecretString" not in properties
        generator = properties["GenerateSecretString"]
...
def test_the_running_backend_cannot_read_the_test_passwords(templates):
    """The app's instance role is granted named resources, not a prefix.

    Worth pinning: the secret sits under the same biofarm/staging/ path as the
    database and Stripe credentials, so a wildcard grant added later would hand
    the backend the keys to its own test accounts without anyone noticing.
    """
    app = templates[f"Biofarm-App-{STAGING.name}"]
    policies = app.find_resources("AWS::IAM::Policy")

    rendered = json.dumps([p["Properties"] for p in policies.values()])
    assert "test-users" not in rendered
